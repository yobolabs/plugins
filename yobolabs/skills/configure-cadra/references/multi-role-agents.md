...
differs; it converts a silent mis-route into a loud failure. Set it with
`outputSchema` on the role (REST, once the origin has p81; **Save as Role** on an
agent also copies one); it goes live on promote (`roles.md` → *Structured output*).
