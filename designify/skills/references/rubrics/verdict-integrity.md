...
## Designify application map

Everything above this line is the spar source, ported verbatim — it is the law and is never edited here. This section adapts *application context* to designify's judge and critic; where a rule names a spar mechanic (spar-state.md, survives/wounded/dead, success.md S2), the row below says what plays that part in a designify mission. The adaptation never overrides the rule text.

| Rule | Spar mechanic | Designify application |
|---|---|---|
| R1 | steelman of the user's position, user-confirmed | **direction steelman**: before the critic attacks a draft or the judge scores a concept low, the artifact's strongest case is stated (best audience-fit + best authenticity argument, evidence included) — the crit report's Steelman section. Attacks or low scores that skip it are invalid. |
| R2 | praise citations in spar rounds | praise in concept scorecards, crit passes, and judge verdicts cites the rubric marker or recon finding that earned it. A reads-real without an R-marker citation is this violation. |
| R3 | survives / wounded / dead + flip condition | the closed vocabularies of this plugin: judge gate **accept / one-more-pass**; concept scoring **strong / adequate / weak** per `concept-selection.md`; crit severities **kills-the-asset / hurts / cosmetic**. Every verdict names its flip condition. Harsh calls standing on `guessing`-tagged evidence are capped pending a recon hunt — the degraded-mode cap applied to creative claims. |
...
| R5 | attack quality floor | the crit quality floor: every finding names its attack class, cites its evidence (trap entry id / M-rule number / recon marker), and states its falsifier — what a passing render would show. Findings missing any of the three are struck as filler. |
| R6 | drift audit across spar rounds | drift audited across the crit re-dispatch, the judge's single revision cycle, and batch execution: a verdict may only change against a specific revision or new evidence; a stricter critic re-dispatch may not flip severities without naming what changed; the ledger's re-grounding checkpoints are R6 applied to assets. |

**Depth theater in designify:** a deepen pass that claims "walked all six lenses" but shows no lens-specific directives; a crit that claims a full trap scan with no trap ids in evidence; a "graded against exemplars" verdict citing none. Same fraud, creative clothes — zero tolerance, especially in blind-pair and replay judging.
